...
void CutID::CutBasedID(int reg,double EFFAr[7][3]){
  ...
  TFile *f1;
  if(reg == 1) f1 =  new TFile("CutTMVABarrel90_test.root","recreate");
  if(reg == 2) f1 =  new TFile("CutTMVAEndCap90_test.root","recreate");
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++){
    ...
    //float PhI = PhPUPPIPhoIso;
    //float ChgI = PhPUPPIChIso;
    //float NeuI = PhPUPPINeuIso;	
