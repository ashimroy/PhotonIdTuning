...
void PlotBuilder(){
  ...
  TFile *finput = new TFile( "/uscms_data/d3/asroy/PhotonIdTuning/CMSSW_7_3_5/src/CutBasedPhoID2016/merged/ChEA_ISOPT/CutTMVAEndCap90_HPT.root");
  ...
  for(int i = 0; i < t_S->GetEntries(); i++){
    ...
    if((sieie  < sieL)&&
       (toe    < 0.05)&&
       (isoP-0.0036*Ppt < isoPL)&&
       (isoC   < isoCL)&&
       (isoN-(0.0153*Ppt+0.000016*Ppt*Ppt) < isoNL)){
    ...
    if((sieie  < sieM)&&
       (toe    < 0.05)&&
       (isoP -0.0036*Ppt < isoPM)&&
       (isoC   < isoCM)&&
       (isoN- (0.0153*Ppt+0.000016*Ppt*Ppt) < isoNM)){
    ...
    if((sieie  < sieT)&&
       (toe    < 0.05)&&
       (isoP -0.0036*Ppt < isoPT)&&
       (isoC   < isoCT)&&
       (isoN - (0.0153*Ppt+0.000016*Ppt*Ppt) < isoNT)){
    ...
    if(isoN-(0.0153*Ppt + 0.000016*Ppt*Ppt) < isoNL){
  ...
  TFile *feta = new TFile("Eff1eta.root","recreate");
  feta->cd();
