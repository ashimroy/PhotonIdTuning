...
void ContourBuilder(int bin,double minEta,double maxEta,double & in,double & ein,double & ip,double & eip,double  & ic,double  & eic){
  ...
  t1->SetBranchAddress("Vtx",&vtx);
  t1->SetBranchAddress("MCVtx",&mcvtx);
  t1->SetBranchAddress("Vty",&vty);
  t1->SetBranchAddress("MCVty",&mcvty);
  t1->SetBranchAddress("Vtz",&vtz);
  t1->SetBranchAddress("MCVtz",&mcvtz);
...
void ErrCalc(TH1F *HIST,int binxn,double perc,double & X_val, double & errXL,double & errXH){
  ...
  // cout<<tot<<endl;
	...
	//eff_err[i -1] = sqrt ( eff[i-1] * ( 1 - eff[i-1] ) / tot ) ;
