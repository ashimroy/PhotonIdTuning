...
void Ex(){
  ...
  //TFile *f1 = new TFile("/uscms_data/d3/asroy/PhotonIdTuning/CMSSW_7_3_5/src/CutBasedPhoID2016/merged/TruePV/PF/Isopt/CutTMVAEndCap90_HPT.root");
  ...
  TH2F *isoPptS = new TH2F("isoPptS","Iso Neutral vs Pt",100,0,1000,1000,0,100);


  for(int i  = 0; i < t_S->GetEntries();i++){
    t_S->GetEntry(i);

    if(ppt < 25 || ppt > 1000) continue;
    if(iso_P == 0) continue;
